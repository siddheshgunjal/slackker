...
def report_stats(client, channel, text, verbose=1):
    ...
    try:
        ...
        if verbose>=1:print(colors.fg.cyan, f"[slackker] Posted message on {channel} channel")

    except SlackApiError as e:
        print(colors.fg.red, f"[slackker] Error posting message: {e}")
        pass
...
def upload_plot(name, client, channel, filepath, verbose=1):
    ...
    try:
        ...
        if verbose>=1:print(colors.fg.cyan, f"[slackker] Uploaded graphs on {channel} channel")

    except SlackApiError as e:
        print(colors.fg.red, f"[slackker] Error uploading graphs: {e}")
        pass
...
def keras_plot_history(modelName, export, client, channel, sendPlot, train_loss, val_loss, train_acc, val_acc, verbose=1):
    ...
    try:
        ...
        if len(train_loss) == 0:
            print(colors.fg.red, '[slackker] Loss is missing in history')
            return 
        ...
        if sendPlot == True:
            try:
                ...
                print(colors.fg.red, f"[slackker] Invalid Argument: {e}")
        else:
            pass
        ...
        print(colors.fg.red, f'[slackker] Plotting Generation Failed: {e}')
        pass
